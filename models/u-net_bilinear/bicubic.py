...
class Bicubic(Layer):
    ...
    def __init__(self, output_dim, **kwargs):
        ...
        super(Bicubic, self).__init__(**kwargs)

    def call(self, tensor, endpoint=False):
        # return tf.image.resize_images(tensor, self.output_dim, tf.image.ResizeMethod.BILINEAR)
        """
        Args :
          tensor : Input tensor. Its shape should be
        ...
        p_00 = tf.gather_nd(tensor, i_00)
        p_10 = tf.gather_nd(tensor, i_10)
        p_20 = tf.gather_nd(tensor, i_20)
        p_30 = tf.gather_nd(tensor, i_30)

        p_01 = tf.gather_nd(tensor, i_01)
        p_11 = tf.gather_nd(tensor, i_11)
        p_21 = tf.gather_nd(tensor, i_21)
        p_31 = tf.gather_nd(tensor, i_31)

        p_02 = tf.gather_nd(tensor, i_02)
        p_12 = tf.gather_nd(tensor, i_12)
        p_22 = tf.gather_nd(tensor, i_22)
        p_32 = tf.gather_nd(tensor, i_32)

        p_03 = tf.gather_nd(tensor, i_03)
        p_13 = tf.gather_nd(tensor, i_13)
        p_23 = tf.gather_nd(tensor, i_23)
        p_33 = tf.gather_nd(tensor, i_33)
